...
const Overlay = styled.div`
  ...
  z-index: 120;
  opacity: ${p => (p.open ? 1 : 0)};  /* open props에 따라 opacity와 pointer-events가 달라짐 */
...
const NavBar = () => {
    ...
    return (
        <>
            ...
            <Overlay open={open} onClick={() => setOpen(false)} />  {/* false/true에 따라 보이는게 결정 */}
            ...
            <DivSidebar open={open}>
                ...
                {/* 클릭하면 navigate()로 페이지 이동 후 사이드바 닫힘 */}
                    ...
                    {/* 섹션: 게시판 */}
                    <SectionTitle>
                        <span><IoList size="20"/></span>
                        <span>게시판</span>
                    </SectionTitle>
                    <MenuItem className="submenu" onClick={() => { navigate('/notice'); setOpen(false); }}>
                        <div className="left">공지 게시판</div>
                        <div className="chev">›</div>
                    </MenuItem>
                    <MenuItem className="submenu" onClick={() => { navigate('/promo'); setOpen(false); }}>
                        <div className="left">홍보 게시판</div>
                        <div className="chev">›</div>
                    </MenuItem>
                    <MenuItem className="submenu" onClick={() => { navigate('/free'); setOpen(false); }}>
                        <div className="left">자유 게시판</div>
                        <div className="chev">›</div>
                    </MenuItem>
                    <MenuItem className="submenu" onClick={() => { navigate('/qna'); setOpen(false); }}>
                    ...
                    <MenuItem className="submenu" onClick={() => { navigate('/popup'); setOpen(false); }}>
