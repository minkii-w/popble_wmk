// src/pages/boards/review/ReviewDetail.jsx  (경로는 프로젝트 구조에 맞춰 조정)
import { useParams, Link, useNavigate } from "react-router-dom";
import { useState, useEffect } from "react";
import { getOne, deleteOne } from "../../../api/BoardApi"; // ✅ 실제 API
...
const ReviewDetail = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError("");
        const data = await getOne(id);                      // ✅ 상세 조회
        if (!ignore) setPost(data);
      } catch (e) {
        if (!ignore) {
          const status = e?.response?.status;
          setError(status === 404 ? "존재하지 않는 리뷰입니다." : "불러오기 실패");
        }
      } finally {
  ...
  const goModify = () => {
    navigate(`/boards/review/${id}/modify`);
  };
  ...
  const onDelete = async () => {                            // ✅ 삭제 핸들러
    ...
    try {
      ...
      await deleteOne(id);
      alert("삭제되었습니다.");
      navigate("/boards/review");                           // ✅ 목록으로 이동
    } catch (e) {
      ...
      {post.images?.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
          {post.images.map((im) => (
            <img
              key={im.id}
              src={im.url}           // BoardApi에서 절대 URL 보정됨
              alt=""
              loading="lazy"
